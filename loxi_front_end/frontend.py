...
def create_member(m_ast, ctx):
    if m_ast[0] == 'pad':
        ...
        if m_ast[2] in ['length', 'len']: # Should be moved to parser
        ...
        raise InputError(f"Dont know how to create member: {m_ast[0]}")
    ...
    for decl_ast in ast:
        if decl_ast[0] == 'struct':
            ...
            if len(discriminators) > 1:
                raise InputError(
                    f"{decl_ast[1]}: Cannot support more than one discriminator by class - got {repr(discriminators)}"
                )
        ...
        if decl_ast[0] == 'enum':
            ...
            enum = ir.OFEnum(
                name=decl_ast[1],
                entries=[
                    ir.OFEnumEntry(name=x[0], value=x[2], params=dict(x[1]))
                    for x in decl_ast[3]
                ],
                params=dict(decl_ast[2]),
            )
