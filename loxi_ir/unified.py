...
def build_unified_ir(name_protocol_map):
        ...
        def add_class(self, version, v_class):
            for v_member in v_class.members:
                if hasattr(v_member, "name"):
                    if v_member.name not in self.members:
                        self.members[v_member.name] = v_member
                    elif type(self.members[v_member.name]) != type(v_member):
                        raise Exception(
                            f"Error unifying ir class {self.name} - adding version: {v_class.protocol.version} - member_type {self.members[v_member.name]} <-> {v_member}"
                        )


            if not self.superclass_set:
                self.superclass_name = v_class.superclass.name if v_class.superclass else None
            elif self.superclass_name != v_class.superclass_name:
                raise Exception(
                    f"Error unifying ir class {self.name} - adding version {v_class.protocol.version} - superclass: param {self.superclass_name} <-> {v_class.superclass_name}"
                )
            ...
            for name, value in v_class.params.items():
                ...
                elif self.params[name] != value:
                    raise Exception(
                        f"Error unifying ir class {self.name} - adding version: {v_class.protocol.version} - param {self.params[name]} <-> {value}"
                    )
            ...
            for e in v_enum.entries:
                if e.name not in self.entries:
                    ...
                    if v_enum.params.get('stable') == 'True' and e.value != entry.value:
                        raise Exception(
                            f"Error unifying stable ir enum {self.name} - adding entry {e.name} version {version} value {entry.value} <-> {e.value}"
                        )
                    ...
                    for name, value in e.params.items():
                        if name not in entry.params:
                            ...
                            raise Exception(
                                f"Error unifying ir enum {self.name} - adding version: param {entry.params[name]} <-> {value}"
                            )
            ...
            for name, value in v_enum.params.items():
                if name not in self.params:
                    ...
                    if name == "wire_type":
                        ...
                        raise Exception(
                            f"Error unifying ir enum {self.name} - adding version: {v_enum.protocol.version} param {self.params[name]} <-> {value}"
                        )
    ...
    for name, spec in u_name_classes.items():
        u = ir.OFUnifiedClass(
            ...
            superclass=unified_classes[spec.superclass_name]
            if spec.superclass_name
            else None,
